...
TOOL_DECLARATIONS = [
    {
        "name": "open_app",
        "description": "Windows'ta uygulama veya web sitesi açar. Spotify, Chrome, Discord vb. uygulamalar; YouTube, Instagram, Gmail vb. web siteleri.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "app_name": {
                    "type": "STRING",
                    "description": "Uygulama adı (örn. 'Spotify', 'Chrome', 'CMD', 'Calculator')"
                }
            },
            "required": ["app_name"]
        }
    ...
    {
        "name": "close_app",
        "description": "Windows'ta açık olan bir uygulamayı kapatır. Spotify, Chrome, Discord, WhatsApp vb.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "app_name": {
                    "type": "STRING",
                    "description": "Kapatılacak uygulama adı (örn. 'Spotify', 'Chrome', 'Discord')"
                }
            },
            "required": ["app_name"]
        }
    ...
    },
    {
        ...
        "description": (
            "Outlook veya Google Takvim etkinliklerini okur. "
    ...
    },
    {
        ...
        "description": (
            "Outlook veya Google Takvime yeni etkinlik ekler. "
    ...
    },
    {
        ...
        "description": (
            "Outlook takviminden etkinlik siler. "
    ...
    },
    {
        "name": "get_reminders",
        "description": (
            "Outlook gorevleri veya hatirlatici listesini okur. "
    ...
    },
    {
        "name": "add_reminder",
        "description": (
            "Outlook gorevleri veya hatirlaticilar listesine yeni bir kayit ekler. "
    ...
    {
        "name": "shell_run",
        "description": "Windows PowerShell veya CMD komutu çalıştırır. Dosya işlemleri, sistem yönetimi.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "command": {
                    "type": "STRING",
                    "description": "Çalıştırılacak Windows komutu (PowerShell veya CMD)"
                }
            },
            "required": ["command"]
        }
    },
    {
        ...
        "description": (
            ...
            "Belirtmezse uygun olanı dene (Spotify varsa önce Spotify, yoksa YouTube). "
    ...
    },
    {
        ...
        "description": (
            ...
            "Windows'ta tam ekran goruntusunu yakalar ve aktif pencere basligini tespit etmeye calisir."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
...
class JarvisLive:
    ...
    def _on_text_command(self, text: str):
        ...
        if not self._loop or not self.session:
            self.ui.write_log("JARVIS henüz bağlanıyor, birkaç saniye bekleyip tekrar dene.")
            return
    ...
    async def _play_audio(self):
        ...
        MIN_BUFFER = RECV_SAMPLE_RATE // 4  # 250ms tampon
        try:
            while True:
                chunk = await self.audio_in_queue.get()
                self.set_speaking(True)
                data_np = np.frombuffer(chunk, dtype="int16").copy()
                buffer = np.concatenate([buffer, data_np])
                # Tampon dolunca yaz — küçük parçalar ses kesintisine yol açar
                while len(buffer) >= MIN_BUFFER:
                    to_write = buffer[:MIN_BUFFER]
